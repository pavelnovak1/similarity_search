...
IT = 13         # Average idle time between flows
TLS = 14        # Time last flow seen
TP = 15         # Ports used by flows
NUMBER_OF_FEATURES = 15

# nfdump -r nfcapd.201311200300_anon -o "fmt: %sa, %da, %td, %pr, %pkt, %byt, %sp, %dp, %tsr, %ter" -N -q -O tend
...
def update_in_stats(data, update):
    ...
    """
    data[IF] += 1
    data = update_idle_time(data, update)
...
def update_idle_time(data, update):
    timeend = float(update.split(',')[9])
    timestart = float(update.split(',')[8])
    if total_flows(data) == 1:
        data[IT] = 0
    else:
        idle_time = count_idle_time(data[TLS], timestart)
        if idle_time < 0:
            None
        else:
            data[IT] = (data[IT]*(total_flows(data)-2) + idle_time)/(total_flows(data) - 1)
    data[TLS] = float(timeend)
    return data

def count_idle_time(lastseen, firstseen):
...
def finish_statistics():
    for record in statistics.keys():
        statistics[record] = statistics[record][:TLS] + statistics[record][TLS + 1:]


def main(raw_data, net='147.250'):
    NET = net
    print("Loading data...")
    parsedata(raw_data)
    print("Data loaded...")
    ...
    finish_statistics()
    print("Done, statistics are ready")
    return statistics
